...
function VRDisableOtherInputs.new(Part: BasePart)
	...
	if UserInputService.VREnabled then
		...
		table.insert(
			self.Events,
			self.Part.DescendantAdded:Connect(function()
				self:UpdateChildren()
			end)
		)
		table.insert(
			self.Events,
			self.Part.DescendantRemoving:Connect(function(Child)
				if not self.Events[Child] then
					return
				end
...
function VRDisableOtherInputs:UpdateChildren(): nil
	for _, Child in self.Part:GetDescendants() do
		if not Child:IsA("ClickDetector") and not Child:IsA("ProximityPrompt") then
			continue
		end
		if self.Events[Child] then
			continue
		end
